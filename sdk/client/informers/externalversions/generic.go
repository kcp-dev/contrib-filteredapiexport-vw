...
func (f *sharedInformerFactory) ForResource(resource schema.GroupVersionResource) (GenericClusterInformer, error) {
	...
	case filteredvwv1alpha1.SchemeGroupVersion.WithResource("filteredapiexportendpointslices"):
		return &genericClusterInformer{resource: resource.GroupResource(), informer: f.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices().Informer()}, nil
	}
...
func (f *sharedScopedInformerFactory) ForResource(resource schema.GroupVersionResource) (GenericInformer, error) {
	...
	// Group=filteredvw.kcp.io, Version=V1alpha1
	case filteredvwv1alpha1.SchemeGroupVersion.WithResource("filteredapiexportendpointslices"):
		informer := f.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices().Informer()
