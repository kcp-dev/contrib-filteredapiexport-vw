...
func (f *sharedInformerFactory) ForResource(resource schema.GroupVersionResource) (GenericClusterInformer, error) {
	...
	case filteredapiexportv1alpha1.SchemeGroupVersion.WithResource("filteredapiexportendpointslices"):
		return &genericClusterInformer{resource: resource.GroupResource(), informer: f.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices().Informer()}, nil
	}
...
func (f *sharedScopedInformerFactory) ForResource(resource schema.GroupVersionResource) (GenericInformer, error) {
	...
	// Group=filteredapiexport.kcp.io, Version=V1alpha1
	case filteredapiexportv1alpha1.SchemeGroupVersion.WithResource("filteredapiexportendpointslices"):
		informer := f.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices().Informer()
