...
import (
	...
	aeadmission "github.com/kcp-dev/contrib-filteredapiexport-vw/virtual/filteredapiexport/admission"
	virtualapiexportauth "github.com/kcp-dev/contrib-filteredapiexport-vw/virtual/filteredapiexport/authorizer"
	"github.com/kcp-dev/contrib-filteredapiexport-vw/virtual/filteredapiexport/controllers/apireconciler"
	"github.com/kcp-dev/contrib-filteredapiexport-vw/virtual/filteredapiexport/schemas"
...
func BuildVirtualWorkspace(
	...
	admissionChain := aeadmission.NewSelectorAdmission(filteredAPIExportInformers.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices(), kcpInformers.Apis().V1alpha2().APIBindings(), kubeClusterClient)
	...
	filteredBoundOrClaimedWorkspaceContent := &virtualdynamic.DynamicVirtualWorkspace{
		...
		BootstrapAPISetManagement: func(mainConfig genericapiserver.CompletedConfig) (apidefinition.APIDefinitionSetGetter, error) {
			...
			apiReconciler, err := apireconciler.NewAPIReconciler(
				kcpClusterClient,
				filteredAPIExportInformers.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices(),
			...
			if err := mainConfig.AddPostStartHook(apireconciler.ControllerName, func(hookContext genericapiserver.PostStartHookContext) error {
				...
				for name, informer := range map[string]cache.SharedIndexInformer{
					...
					"filteredapiexportendpointslices": filteredAPIExportInformers.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices().Informer(),
				} {
		...
		Authorizer: newAuthorizer(kubeClusterClient, deepSARClient, cachedKcpInformers, kcpInformers, filteredAPIExportInformers),
...
func newAuthorizer(kubeClusterClient, deepSARClient kcpkubernetesclientset.ClusterInterface, cachedKcpInformers, kcpInformers kcpinformers.SharedInformerFactory, filteredAPIExportInformers filteredapiexportinformers.SharedInformerFactory) authorizer.Authorizer {
	maximalPermissionAuth := virtualapiexportauth.NewMaximalPermissionAuthorizer(deepSARClient, filteredAPIExportInformers.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices(), cachedKcpInformers.Apis().V1alpha2().APIExports())
	...
	boundApiAuth := virtualapiexportauth.NewBoundAPIAuthorizer(apiExportsContentAuth, filteredAPIExportInformers.Filteredapiexport().V1alpha1().FilteredAPIExportEndpointSlices(), kcpInformers.Apis().V1alpha2().APIBindings(), cachedKcpInformers.Apis().V1alpha2().APIExports(), kubeClusterClient)
