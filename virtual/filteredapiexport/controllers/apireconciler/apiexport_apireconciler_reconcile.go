...
import (
	...
	apiexportbuiltin "github.com/kcp-dev/contrib-filteredapiexport-vw/virtual/filteredapiexport/schemas/builtin"
...
func (c *APIReconciler) reconcile(ctx context.Context, filteredSlice *filteredapiexportv1alpha1.FilteredAPIExportEndpointSlice, apiDomainKey dynamiccontext.APIDomainKey) error {
	...
	if filteredSlice != nil {
		apiExport, err = c.apiExportLister.Cluster(logicalcluster.From(filteredSlice)).Get(filteredSlice.Spec.APIExport.Name)
		if err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}

	if filteredSlice == nil || apiExport == nil || apiExport.Status.IdentityHash == "" {
		c.mutex.RLock()
	...
	claimFilteredAPIExportES := false
	for _, pc := range apiExport.Spec.PermissionClaims {
		...
		if pc.Group == filteredapiexportv1alpha1.GroupName {
			apisSchema, found := schemas.ApisKcpDevSchemas[pc.Resource]
			if !found {
				logger.Info("permission claim is for an unknown resource", "claim", pc)
				continue
			...
			if pc.Resource == "filteredapiexportendpointslices" {
				claimFilteredAPIExportES = true
			}
	...
	for _, apiResourceSchema := range apiResourceSchemas {
		for _, version := range apiResourceSchema.Spec.Versions {
			...
			selector, err := metav1.LabelSelectorAsSelector(&filteredSlice.Spec.ObjectSelector.LabelSelector)
			if err != nil {
				return err
			}
			labelReqs, selectable = selector.Requirements()
			if !selectable {
				return fmt.Errorf("unable to create a selector from the provided labels in the FilteredAPIExportEndpointSlice")
			}
			...
			if err != nil {
				// TODO: would be nice to expose some sort of user-visible error
	...
	if !claimFilteredAPIExportES {
		gvr := filteredapiexportv1alpha1.SchemeGroupVersion.WithResource("filteredapiexportendpointslices")
		d, err := c.createFilteredAPIExportESAPIDefinition(ctx, gvr.Version, clusterName, apiExport.Name)
		if err != nil {
			// TODO: would be nice to expose some sort of user-visible error
			logger.Error(err, "error creating api definition for filteredapiexportendpointslices")
		}
