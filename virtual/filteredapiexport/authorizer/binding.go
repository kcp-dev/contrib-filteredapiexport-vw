...
func NewBoundAPIAuthorizer(delegate authorizer.Authorizer, filteredAPIExportESInformer filteredapiexportinformers.FilteredAPIExportEndpointSliceClusterInformer, apiBindingInformer apisv1alpha2informers.APIBindingClusterInformer, apiExportInformer apisv1alpha2informers.APIExportClusterInformer, kubeClusterClient kcpkubernetesclientset.ClusterInterface) authorizer.Authorizer {
	...
	return &boundAPIAuthorizer{
		...
		getAPIBindingByExport: func(clusterName, apiExportName, apiExportCluster string) (*apisv1alpha2.APIBinding, error) {
			...
			return nil, fmt.Errorf("no suitable binding found 2")
		},
	}
}
...
func (a *boundAPIAuthorizer) Authorize(ctx context.Context, attr authorizer.Attributes) (authorizer.Decision, string, error) {
	...
	apiExport, err := a.getAPIExport(filteredAPIExportES.Spec.APIExport.Path, filteredAPIExportES.Spec.APIExport.Name)
	if apierrors.IsNotFound(err) {
	...
	apiBinding, err := a.getAPIBindingByExport(targetCluster.Name.String(), apiExport.Name, filteredAPIExportES.Spec.APIExport.Path)
	if err != nil {
	...
	// special case: APIBindings are always available from a FilteredAPIExport VW,
