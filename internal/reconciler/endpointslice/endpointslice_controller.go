...
import (
	...
	filteredvwindexers "github.com/kcp-dev/contrib-filteredapiexport-vw/internal/indexers"
	...
	filteredvwclient "github.com/kcp-dev/contrib-filteredapiexport-vw/sdk/client/clientset/versioned/typed/filteredvw/v1alpha1"
...
func NewController(
	...
	filteredAPIExportClusterClient filteredvwclientset.ClusterInterface,
	...
	c := &controller{
		...
		commit: committer.NewCommitter[*FilteredAPIExportEndpointSlice, Patcher, *FilteredAPIExportEndpointSliceSpec, *FilteredAPIExportEndpointSliceStatus](filteredAPIExportClusterClient.FilteredvwV1alpha1().FilteredAPIExportEndpointSlices()),
	}
...
type FilteredAPIExportEndpointSlice = filteredvwv1alpha1.FilteredAPIExportEndpointSlice
type FilteredAPIExportEndpointSliceSpec = filteredvwv1alpha1.FilteredAPIExportEndpointSliceSpec
type FilteredAPIExportEndpointSliceStatus = filteredvwv1alpha1.FilteredAPIExportEndpointSliceStatus
type Patcher = filteredvwclient.FilteredAPIExportEndpointSliceInterface
...
func (c *controller) enqueueFilteredAPIExportEndpointSlice(slice *filteredvwv1alpha1.FilteredAPIExportEndpointSlice, logger logr.Logger, logSuffix string) {
...
func (c *controller) enqueueFilteredAPIExportEndpointSlicesForAPIExport(export *apisv1alpha2.APIExport, logger logr.Logger) {
	...
	if path := logicalcluster.NewPath(export.Annotations[core.LogicalClusterPathAnnotationKey]); !path.Empty() {
		pathKeys, err := c.filteredAPIExportEndpointSliceClusterInformer.Informer().GetIndexer().IndexKeys(filteredvwindexers.FilteredAPIExportEndpointSliceByAPIExport, path.Join(export.Name).String())
		if err != nil {
			utilruntime.HandleError(err)
			return
		}
	...
	clusterKeys, err := c.filteredAPIExportEndpointSliceClusterInformer.Informer().GetIndexer().IndexKeys(filteredvwindexers.FilteredAPIExportEndpointSliceByAPIExport, logicalcluster.From(export).Path().Join(export.Name).String())
	if err != nil {
		utilruntime.HandleError(err)
		return
	}
	...
	for _, key := range sets.List[string](keys) {
		...
		c.enqueueFilteredAPIExportEndpointSlice(tombstone.Obj[*filteredvwv1alpha1.FilteredAPIExportEndpointSlice](slice), logger, " because of referenced FilteredAPIExport")
	}
}
...
func InstallIndexers(
	...
	indexers.AddIfNotPresentOrDie(filteredAPIExportEndpointSliceClusterInformer.Informer().GetIndexer(), cache.Indexers{
		filteredvwindexers.FilteredAPIExportEndpointSliceByAPIExport: filteredvwindexers.IndexFilteredAPIExportEndpointSliceByAPIExport,
	})
}
