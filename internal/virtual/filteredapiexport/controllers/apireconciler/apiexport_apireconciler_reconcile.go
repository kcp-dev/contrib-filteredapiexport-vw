...
import (
	...
	apiexportbuiltin "github.com/kcp-dev/contrib-filteredapiexport-vw/internal/virtual/filteredapiexport/schemas/builtin"
...
func (c *APIReconciler) reconcile(ctx context.Context, filteredSlice *filteredvwv1alpha1.FilteredAPIExportEndpointSlice, apiDomainKey dynamiccontext.APIDomainKey) error {
	...
	for _, pc := range apiExport.Spec.PermissionClaims {
		...
		if pc.Group == filteredvwv1alpha1.GroupName {
			apisSchema, found := schemas.ApisKcpDevSchemas[pc.Resource]
			if !found {
				logger.Info("permission claim is for an unknown resource", "claim", pc)
				continue
	...
	if !claimFilteredAPIExportES {
		gvr := filteredvwv1alpha1.SchemeGroupVersion.WithResource("filteredapiexportendpointslices")
