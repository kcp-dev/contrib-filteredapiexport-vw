...
import (
	...
	aeadmission "github.com/kcp-dev/contrib-filteredapiexport-vw/internal/virtual/filteredapiexport/admission"
	virtualapiexportauth "github.com/kcp-dev/contrib-filteredapiexport-vw/internal/virtual/filteredapiexport/authorizer"
	"github.com/kcp-dev/contrib-filteredapiexport-vw/internal/virtual/filteredapiexport/controllers/apireconciler"
	"github.com/kcp-dev/contrib-filteredapiexport-vw/internal/virtual/filteredapiexport/schemas"
	filteredvwinformers "github.com/kcp-dev/contrib-filteredapiexport-vw/sdk/client/informers/externalversions"
...
const (
	...
	VirtualWorkspaceName string = "filtered-apiexport"
...
func BuildVirtualWorkspace(
	...
	filteredvwinformers filteredvwinformers.SharedInformerFactory,
	...
	admissionChain := aeadmission.NewSelectorAdmission(filteredvwinformers.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices(), kcpInformers.Apis().V1alpha2().APIBindings(), kubeClusterClient)

	filteredBoundOrClaimedWorkspaceContent := &virtualdynamic.DynamicVirtualWorkspace{
		...
		ReadyChecker: framework.ReadyFunc(func() error {
			...
			case <-readyCh:
				return nil
			default:
				return errors.New("filtered-apiexport virtual workspace controllers are not started")
			}
		}),
		...
		BootstrapAPISetManagement: func(mainConfig genericapiserver.CompletedConfig) (apidefinition.APIDefinitionSetGetter, error) {
			...
			apiReconciler, err := apireconciler.NewAPIReconciler(
				kcpClusterClient,
				filteredvwinformers.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices(),
			...
			if err := mainConfig.AddPostStartHook(apireconciler.ControllerName, func(hookContext genericapiserver.PostStartHookContext) error {
				...
				for name, informer := range map[string]cache.SharedIndexInformer{
					...
					"filteredapiexportendpointslices": filteredvwinformers.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices().Informer(),
				} {
		...
		Authorizer: newAuthorizer(kubeClusterClient, deepSARClient, cachedKcpInformers, kcpInformers, filteredvwinformers),
		Mutator:    admissionChain,
		Validator:  admissionChain,
	}

	return []rootapiserver.NamedVirtualWorkspace{
		{Name: VirtualWorkspaceName, VirtualWorkspace: filteredBoundOrClaimedWorkspaceContent},
	}, nil
}
...
func digestUrl(urlPath, rootPathPrefix string) (
	...
	//  /services/filtered-apiexport/root:org:ws/<filtered-apiexport-endpointslice>/clusters/*/api/v1/configmaps
	//                              └───────────────┐
	...
	filteredAPIExportESClusterName, filteredAPIExportESName := parts[0], parts[1]
	if filteredAPIExportESClusterName == "" {
		return genericapirequest.Cluster{}, "", "", false
	}
	if filteredAPIExportESName == "" {
		return genericapirequest.Cluster{}, "", "", false
	}
	...
	key := fmt.Sprintf("%s/%s", filteredAPIExportESClusterName, filteredAPIExportESName)
	return cluster, dynamiccontext.APIDomainKey(key), strings.TrimSuffix(urlPath, realPath), true
}

func newAuthorizer(kubeClusterClient, deepSARClient kcpkubernetesclientset.ClusterInterface, cachedKcpInformers, kcpInformers kcpinformers.SharedInformerFactory, filteredvwinformers filteredvwinformers.SharedInformerFactory) authorizer.Authorizer {
	maximalPermissionAuth := virtualapiexportauth.NewMaximalPermissionAuthorizer(deepSARClient, filteredvwinformers.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices(), cachedKcpInformers.Apis().V1alpha2().APIExports())
	maximalPermissionAuth = authorization.NewDecorator("virtual.filtered-apiexport.maxpermissionpolicy.authorization.kcp.io", maximalPermissionAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()

	apiExportsContentAuth := virtualapiexportauth.NewFilteredAPIExportsContentAuthorizer(maximalPermissionAuth, kubeClusterClient)
	apiExportsContentAuth = authorization.NewDecorator("virtual.filtered-apiexport.content.authorization.kcp.io", apiExportsContentAuth).AddAuditLogging().AddAnonymization()

	boundApiAuth := virtualapiexportauth.NewBoundAPIAuthorizer(apiExportsContentAuth, filteredvwinformers.Filteredvw().V1alpha1().FilteredAPIExportEndpointSlices(), kcpInformers.Apis().V1alpha2().APIBindings(), cachedKcpInformers.Apis().V1alpha2().APIExports(), kubeClusterClient)
	boundApiAuth = authorization.NewDecorator("virtual.filtered-apiexport.boundapi.authorization.kcp.io", boundApiAuth).AddAuditLogging().AddAnonymization()
