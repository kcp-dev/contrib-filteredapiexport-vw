...
func init() {
	...
	// TODO(xmudrii): remove once we have the filteredapiexportendpointslices CRD in config/crds
	// get APIBindings resource schema
	crd := apiextensionsv1.CustomResourceDefinition{}
	if err := yaml.Unmarshal([]byte(configFile), &crd); err != nil {
		panic(fmt.Sprintf("failed to unmarshal apibindings resource: %v", err))
	}
	schema, err := apisv1alpha1.CRDToAPIResourceSchema(&crd, "crd")
	if err != nil {
	...
	ApisKcpDevSchemas["filteredapiexportendpointslices"] = schema
}
